...
export default function IntakesPage() {
  ...
  const [linkCheck, setLinkCheck] = useState<"idle" | "checking" | "ready" | "failed">("idle");
  ...
  async function create(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setError(""); setCreatedUrl(""); setLinkCheck("idle");
  ...
  async function verifyCreatedLink() {
    setLinkCheck("checking");
    try {
      const response = await fetch(createdUrl, { redirect: "follow", cache: "no-store" });
      setLinkCheck(response.ok && response.url.includes("/relevamiento/") ? "ready" : "failed");
    } catch {
      setLinkCheck("failed");
    }
  }
