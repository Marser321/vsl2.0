...
Estado al 2026-07-11. Lo construido y el backlog de ideas laterales priorizado.
...
- **Pase visual pendiente de ejecutar por GPT**: brief autocontenido en `docs/briefs/gpt-pase-visual.md` (correr sobre main estable).

## Backlog de ideas laterales (orden sugerido)

1. **Métricas reales de plataformas → ratings automáticos.** Pegar stats de Meta/TikTok (retención 3s, CTR, CPA) en el guion; convertirlas en señal dura que complemente las ★ del equipo. La versión con mejores métricas reales se auto-promueve como candidata a ganadora. *Por qué: cierra el loop con datos de mercado, no solo criterio interno.*
2. **Importador de transcripts de YouTube en el Analizador.** La extracción de captions ya existe (`src/lib/ingest/url.ts` extrae `captionTracks`); falta exponerla en `/analizador` (pegar URL → transcript → análisis estructural → biblioteca). *Esfuerzo bajo, la mitad del código ya está.*
3. **Reescritura guiada por mapa de retención.** En el teleprompter ya se detectan fugas heurísticas; agregar "corregir esta sección con IA" que mande la sección + flag al refinador. *Une dos features existentes.*
4. **Radar programado (cron semanal).** El radar hoy es manual; un cron (Vercel Cron → route handler) lo corre lunes temprano para todos los clientes con industria definida y avisa por email (Resend ya está integrado).
